...
Page({
  ...
  onLoad(options) {
    wx.cloud.callFunction({
      ...
      success: res => {
        console.log(res)  
        const fabuhuiData = res.result.data.fabuhuiData.data
        const kaiyeData = res.result.data.kaiyeData.data
        const wanhuiData = res.result.data.wanhuiData.data
        const yanlianData = res.result.data.yanlianData.data
        const activityOthers = res.result.data.activityOthers.data[0]
        const activityVideo = res.result.data.activityVideo.data

        let kaiyeV,wanhuiV,yanlianV,fabuhuiV;
        activityVideo.map((v,i)=>{
          if(v.type==='fabuhui') {
            fabuhuiV = v
          }
          if(v.type==='kaiye') {
        ...
        console.log(kaiyeV,wanhuiV,yanlianV,fabuhuiV)
        this.setData({
          fabuhuiData,
          kaiyeData,
          wanhuiData,
